...
Comprehensive health checks for Nginx, Vite, React, Express server setups
"""
...
class SanityChecker:
    ...
    async def run_all_checks(self) -> SanityReport:
        ...
        await self._check_vite()
        await self._check_express()
    ...
    async def _check_vite(self):
        ...
        vite_configs = [
            ...
            self.project_path / "vite.config.mjs",
        ]
        ...
        if vite_config:
            self._add_check(CheckResult(
                name="Vite Config",
                category=CheckCategory.VITE,
                status=CheckStatus.PASS,
                message=f"Found {vite_config.name}",
                duration_ms=(time.time() - start) * 1000
            ))
            ...
            try:
                ...
                if "build:" not in content and "build :" not in content:
                    suggestions.append("Add build configuration for production optimization")
                
                if "sourcemap" not in content.lower():
                    suggestions.append("Consider configuring sourcemaps for debugging")
                
                if "minify" not in content.lower():
                    suggestions.append("Consider explicit minify configuration")
                
                if "chunkSizeWarningLimit" not in content:
                    suggestions.append("Set chunkSizeWarningLimit to control bundle size warnings")
                
                if suggestions:
                    self._add_check(CheckResult(
                        name="Vite Config Analysis",
                        category=CheckCategory.VITE,
                        status=CheckStatus.WARN,
                        message=f"{len(suggestions)} optimization suggestions",
                        details="\n".join(f"• {s}" for s in suggestions),
                        suggestion="Review vite.config for production optimizations."
                    ))
                else:
                    self._add_check(CheckResult(
                        name="Vite Config Analysis",
                        category=CheckCategory.VITE,
                        status=CheckStatus.PASS,
                        message="Config looks well optimized",
                        duration_ms=(time.time() - start) * 1000
                    ))
            except Exception as e:
                self._add_check(CheckResult(
                    ...
                    category=CheckCategory.VITE,
                    status=CheckStatus.SKIP,
                    message=f"Could not analyze config: {str(e)}"
                ))
        else:
                ...
                try:
                    ...
                    if "vite" in deps:
                        self._add_check(CheckResult(
                            ...
                            category=CheckCategory.VITE,
                            status=CheckStatus.WARN,
                            message="Vite installed but no config file found",
                            suggestion="Create vite.config.ts for custom configuration.",
                            fix_command="# Create vite.config.ts with: npx vite --config"
                        ))
                    else:
                        self._add_check(CheckResult(
                            name="Vite Config",
        ...
        if dist_folder.exists():
            ...
            if js_files or css_files:
                ...
                self._add_check(CheckResult(
                    ...
                    category=CheckCategory.VITE,
                    status=CheckStatus.PASS,
                    message=f"dist/ exists: {len(js_files)} JS, {len(css_files)} CSS files ({total_size/1024/1024:.1f}MB)",
                    duration_ms=(time.time() - start) * 1000
                ))
            else:
                self._add_check(CheckResult(
                    ...
                    category=CheckCategory.VITE,
                    status=CheckStatus.WARN,
                    message="dist/ exists but appears empty",
                    suggestion="Run 'pnpm build' to generate production build."
                ))
        else:
            self._add_check(CheckResult(
                name="Vite Build Output",
    ...
    async def _check_express(self):
        """Express server checks"""
        import time
            ...
            try:
                ...
                if has_express:
                    self._add_check(CheckResult(
                        ...
                        category=CheckCategory.EXPRESS,
                        status=CheckStatus.PASS,
                        message=f"Express {deps.get('express', 'installed')}",
                        duration_ms=(time.time() - start) * 1000
                    ))
                else:
                    self._add_check(CheckResult(
                        ...
                        category=CheckCategory.EXPRESS,
                        status=CheckStatus.SKIP,
                        message="Express not in dependencies"
                    ))
            except Exception as e:
                self._add_check(CheckResult(
                    name="Express Installed",
        ...
        server_files = [
            ...
            "src/server.js", "src/server.ts", "src/server.mjs",
        ...
        for sf in server_files:
            if (self.project_path / sf).exists():
                found_server = sf
                break
        ...
        if found_server:
            ...
            start = time.time()
            try:
                content = (self.project_path / found_server).read_text()
                issues = []
                
                # Check for common issues
                if "cors" not in content.lower():
                    issues.append("CORS middleware not detected - may cause cross-origin issues")
                
                if "helmet" not in content.lower():
                    issues.append("Helmet not detected - consider adding for security headers")
                
                if "compression" not in content.lower():
                    issues.append("Compression middleware not detected - consider for performance")
                
                if "error" not in content.lower() or "catch" not in content.lower():
                    issues.append("Error handling may be incomplete")
                ...
                if issues:
                    self._add_check(CheckResult(
                        ...
                        category=CheckCategory.EXPRESS,
                        status=CheckStatus.WARN,
                        message=f"{len(issues)} suggestions found",
                        details="\n".join(f"• {i}" for i in issues),
                        suggestion="Review Express middleware configuration."
                    ))
                else:
                    self._add_check(CheckResult(
                        name="Express Security/Performance",
            ...
            self._add_check(CheckResult(
                ...
                suggestion="Create server.js or server.ts as Express entry point."
            ))
        
        # Check if Express server is running (common ports)
        start = time.time()
        express_ports = [3000, 3001, 4000, 5000, 8000, 8080]
        running_port = None
        ...
        if running_port:
            self._add_check(CheckResult(
                ...
                category=CheckCategory.EXPRESS,
                status=CheckStatus.PASS,
                message=f"Server detected on port {running_port}",
                duration_ms=(time.time() - start) * 1000
            ))
        else:
            self._add_check(CheckResult(
                ...
                category=CheckCategory.EXPRESS,
                status=CheckStatus.SKIP if self.environment == "dev" else CheckStatus.WARN,
                message="No Express server detected on common ports",
                suggestion="Start the server with 'pnpm dev' or 'node server.js'"
            ))
