...
# Deploy Updated Frontend with Certificate Management
# This script deploys the updated frontend to the Ubuntu server
...
echo -e "${BLUE}🚀 Deploying Updated Frontend with Certificate Management${NC}"
echo "======================================================"
...
# Check if we have the updated files in the current directory
if [ -f "./web/dist/index.html" ]; then
    # Copy from local build
    cp -r ./web/dist/* "$WEB_DIST_DIR/"
    echo -e "${GREEN}✅ Frontend files copied from local build${NC}"
elif [ -f "/tmp/web/dist/index.html" ]; then
    # Copy from temp directory
    cp -r /tmp/web/dist/* "$WEB_DIST_DIR/"
    echo -e "${GREEN}✅ Frontend files copied from temp directory${NC}"
else
    echo -e "${RED}❌ Updated frontend files not found${NC}"
    echo "Please ensure the frontend has been built and is available:"
    echo "  - ./web/dist/ (if running from project root)"
    echo "  - /tmp/web/dist/ (if files were copied to temp)"
    exit 1
fi
